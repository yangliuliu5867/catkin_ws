...
ros::Publisher droneArrayPub;  // 添加无人机数组发布者
ros::Publisher armsPub;
ros::Publisher propsPub;
...
void fov_visual_init(std::string msg_frame_id)
{
  ...
  markerEdge_fov.action = visualization_msgs::Marker::LINE_LIST;
...
void odom_callback(const nav_msgs::Odometry::ConstPtr &msg)
{
  ...
  // 科研风格简化无人机模型 - 淡灰色系，基于实际参数
  // 实际参数：轴距0.25m，螺旋桨半径0.062m
  visualization_msgs::MarkerArray drone_markers;
  
  // 主体 - 扁长方体 (机身) 
  visualization_msgs::Marker body;
  body.header.frame_id = _frame_id;
  body.header.stamp = msg->header.stamp;
  body.ns = "drone_body";
  body.id = 0;
  body.type = visualization_msgs::Marker::CUBE;
  body.action = visualization_msgs::Marker::ADD;
  body.pose.position.x = pose(0);
  body.pose.position.y = pose(1);
  body.pose.position.z = pose(2);
  ...
  body.pose.orientation.w = q(0);
  body.pose.orientation.x = q(1);
  body.pose.orientation.y = q(2);
  body.pose.orientation.z = q(3);
  
  // 机身尺寸: 基于实际轴距调整
  double arm_length = 0.25;  // 实际轴距
  body.scale.x = scale * arm_length * 0.6;  // 机身长度 (轴距的60%)
  body.scale.y = scale * arm_length * 0.3;  // 机身宽度 (轴距的30%)
  body.scale.z = scale * arm_length * 0.15; // 机身高度 (轴距的15%)
  
  // 淡灰色机身
  body.color.a = 0.9;  // 确保不透明度足够
  body.color.r = 0.7;  // 淡灰色
  body.color.g = 0.7;
  body.color.b = 0.7;
  
  drone_markers.markers.push_back(body);
  
  // 螺旋桨臂 - 4根细长方体，真正的X形布局
  for (int i = 0; i < 4; i++) {
    visualization_msgs::Marker arm;
    arm.header.frame_id = _frame_id;
    ...
    // X形机架：45度角分布
    double angle = i * M_PI / 2.0 + M_PI / 4.0;  // 45, 135, 225, 315度
    double arm_length_half = scale * arm_length * 0.4;  // 臂长度的一半
    
    // 臂的中心位置（从无人机中心到臂中点）
    double local_x = arm_length_half * 0.5 * cos(angle);
    double local_y = arm_length_half * 0.5 * sin(angle);
    
    colvec arm_local(3);
    arm_local(0) = local_x;
    arm_local(1) = local_y; 
    arm_local(2) = 0;
    mat R = quaternion_to_R(q);
    colvec arm_global = R * arm_local;
    
    arm.pose.position.x = pose(0) + arm_global(0);
    arm.pose.position.y = pose(1) + arm_global(1);
    arm.pose.position.z = pose(2) + arm_global(2);
    
    // 设置臂的方向，使其沿着X形的方向延伸
    colvec arm_ypr = R_to_ypr(quaternion_to_R(q));
    arm_ypr(0) += angle;  // 加上臂的角度
    colvec arm_q = R_to_quaternion(ypr_to_R(arm_ypr));
    
    arm.pose.orientation.w = arm_q(0);
    arm.pose.orientation.x = arm_q(1);
    arm.pose.orientation.y = arm_q(2);
    arm.pose.orientation.z = arm_q(3);
    
    // 臂的尺寸 - 更长更细以突出X形
    arm.scale.x = scale * arm_length * 0.8;  // 臂长度
    arm.scale.y = scale * arm_length * 0.04; // 臂宽度（更细）
    arm.scale.z = scale * arm_length * 0.04; // 臂高度（更薄）
    ...
    drone_markers.markers.push_back(arm);
  }

   // 螺旋桨 - 4个扁圆柱，使用实际尺寸
  double prop_radius = 0.062;  // 实际螺旋桨半径
  for (int i = 0; i < 4; i++) {
    visualization_msgs::Marker prop;
    prop.header.frame_id = _frame_id;
    prop.header.stamp = msg->header.stamp;
    prop.ns = "drone_props";
    prop.id = i;
    prop.type = visualization_msgs::Marker::CYLINDER;
    prop.action = visualization_msgs::Marker::ADD;
    
    // 螺旋桨位置 (在臂的末端，X形布局)
    double angle = i * M_PI / 2.0 + M_PI / 4.0;  // 45, 135, 225, 315度
    double prop_distance = scale * arm_length * 0.5;  // 螺旋桨到中心距离（轴距的一半）
    
    double local_x = prop_distance * cos(angle);
    double local_y = prop_distance * sin(angle);
    
    colvec prop_local(3);
    prop_local(0) = local_x;
    prop_local(1) = local_y;
    prop_local(2) = scale * arm_length * 0.1;  // 略高于机身
    mat R = quaternion_to_R(q);
    colvec prop_global = R * prop_local;
    
    prop.pose.position.x = pose(0) + prop_global(0);
    prop.pose.position.y = pose(1) + prop_global(1);
    prop.pose.position.z = pose(2) + prop_global(2);
    prop.pose.orientation.w = q(0);
    prop.pose.orientation.x = q(1);
    prop.pose.orientation.y = q(2);
    prop.pose.orientation.z = q(3);
    
    // 螺旋桨尺寸 - 使用实际尺寸，不放大
    prop.scale.x = scale * prop_radius * 2.0;  // 直径（去掉1.5倍放大）
    prop.scale.y = scale * prop_radius * 2.0;  // 直径
    prop.scale.z = scale * prop_radius * 0.05; // 厚度（更薄一些）
    
    // 螺旋桨颜色 (半透明浅灰色)
    prop.color.a = 0.7;
    prop.color.r = 0.8;  // 浅灰色
    prop.color.g = 0.8;
    prop.color.b = 0.8;
    
    drone_markers.markers.push_back(prop);
  }

  // 方向指示器 - 前方箭头
  visualization_msgs::Marker direction;
  direction.header.frame_id = _frame_id;
  direction.header.stamp = msg->header.stamp;
  direction.ns = "drone_direction";
  direction.id = 0;
  ...
  // 前方向量
  colvec forward_local(3);
  forward_local(0) = scale * arm_length * 1.2;  // 前方箭头长度
  forward_local(1) = 0;
  forward_local(2) = 0;
  mat R = quaternion_to_R(q);
  colvec forward_global = R * forward_local;
  
  end.x = pose(0) + forward_global(0);
  end.y = pose(1) + forward_global(1);
  end.z = pose(2) + forward_global(2);
  
  direction.points.push_back(start);
  direction.points.push_back(end);
  
  direction.scale.x = scale * arm_length * 0.05;  // 箭头杆直径
  direction.scale.y = scale * arm_length * 0.10;  // 箭头头部直径
  direction.scale.z = scale * arm_length * 0.12;  // 箭头头部长度
  
  // 方向箭头颜色 (淡蓝色指示前方)
  direction.color.a = 0.8;
  direction.color.r = 0.4;
  direction.color.g = 0.6;
  direction.color.b = 0.9;  // 淡蓝色
  
  drone_markers.markers.push_back(direction);
  
  // 发布完整的无人机模型
  droneArrayPub.publish(drone_markers);
  
  // 同时发布机身到原有话题保持兼容性
  meshROS = body;  // 使用body数据
...
int main(int argc, char **argv)
{
  ...
  droneArrayPub = n.advertise<visualization_msgs::MarkerArray>("robot_array", 100, true);
